...
- **文生图四档提示词模式与请求级控制**：`/g2生图` 支持 4 档提示词处理模式：`off`（原文直传）、`extract`（仅提取比例/分辨率）、`standard`（精准整理，20~45 词）、`enhance`（受控增强，45~80 词），支持在命令中以短参数 `-off`、`-ex`、`-st`、`-eh`（及长参数 `--off` 等）覆盖单次生图请求模式。
- **自定义风格预设体系 (`-ys<名称>`) 与 WebUI Pages 管理面板**：新增预设调用参数 `-ys<名称>`（如 `-ys二次元`、`-ys电影质感`），与内置模式互斥。支持在 WebUI Pages（`pages/presets/`）和配置中自由添加、编辑、导出、测试与管理自定义风格指令，提供三段式结构组装实时模拟器。
...
- **候选模型多轮轮询（来回重试）调度重构**：模型重试机制重构为“单次失败立即切换下一个候选模型，多候选轮询来回重试（总轮次数对齐 `1 + model_retry_count`，视频对齐 `1 + video_retry_count`）”，避免单一模型原地重试造成的任务阻塞与延迟，提升高可用性与响应效率。

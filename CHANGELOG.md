...
- **自定义风格预设体系 (`-ys<名称>`)**：新增预设调用参数 `-ys<名称>`（如 `-ys二次元`、`-ys电影质感`），与内置模式互斥。支持在 AstrBot 原生配置面板中自由添加、编辑与管理自定义风格指令，内置开箱即用的「二次元」与「电影质感」预设。
- **多模型重试策略开关 (`model_retry_strategy`)**：支持在 WebUI 中选择「轮询重试」（`round_robin`，默认）或「依次重试」（`sequential`），统一驱动搜索、生图、改图和视频候选模型切换。
- **三段式 System Prompt 架构与精确词数约束**：系统提示词统一收敛为「公共保真底座（顶部） + 模式/预设专属指令（中间） + JSON 输出规范与标准示例（底部）」三段式架构，严格杜绝指令矛盾，`standard` 模式严格约束 20~45 词，`enhance` 模式严格约束 45~80 词。
- **未识别参数前置拦截**：`/g2生图`、`/g2改图`、`/g2视频` 剥离全部合法参数后，若仍存在以 `-` 或 `--` 加英文字母开头的 token（例如大小写不符的 `-EH`、其他工具的 `--ar 16:9`），将在任何提示词改写、资料搜索与上游生成请求之前拒绝，并回复未识别的参数与该命令的可用参数列表（如 `-off、-ex、-st、-eh、-ys[预设名]、-s`），不会被静默当作提示词内容发送。普通文本中的连字符不受影响（`-`、`--`、`-5`、`-可爱`、`T-shirt`、`COVID-19` 等仍按原文处理）。
- **生图专用通用视觉事实检索与显式搜索开关 (`-s`/`--search`)**：新增 `character_research_mode`（`off`/`auto`/`always`）与 `character_research_timeout_seconds`。在 `standard`、`enhance` 或 `-ys<名称>` 预设模式下，`/g2生图` 可自动或通过 `-s`/`--search` 显式检索具名角色、IP、现实物品、建筑、载具等实体的视觉外观事实，清洗后注入改写模型。提示词处理与视觉事实检索**严格仅服务于 `/g2生图`**。
- **显式控制硬失败保障与配置回退自愈**：当使用显式模式参数（如 `-eh`、`-ys二次元`）或显式搜索标记（`-s`/`--search`）时，若改写模型异常、显式搜索失败/超时/无资料，任务将立即中止并报告错误，绝不静默回退原文；仅在使用 WebUI 默认配置模式且未显式指定时，才按 `fallback_to_original_on_error` 降级为原文直传。
- **搜索 Tool 极简重构与无状态纯净排版**：彻底移除单任务配额限制与内部计数器，会话级大模型 Tool（`grok2api_web_search`）采用极简无状态设计并向 LLM 返回纯净排版文本（`format_search_for_llm`），彻底根除大模型复述“到达上限”的提示。
- **正文 URL 正则提取兜底**：上游未提供结构化来源字段时，自动使用 URL 正则从模型正文中清洗提取网址并作为来源兜底填充。
- **配置面板 8 大职责分组与 v3 自动迁移**：将 AstrBot WebUI 插件配置按职责拆分为连接设置、图片与视频、提示词处理、联网搜索、访问控制、性能与可靠性、文件与缓存、管理面板 8 个分组；内置 `config_layout_version` 升级至 3，自动将旧版配置中严格无损行为的 `enhance` 迁移为 `standard`，并平滑降级旧版 `enhance_pro` 至 `enhance`。

### Changed

- **单用户并发任务隔离与同群多人群友并发**：将原本粗暴锁定整个群会话的锁机制升级为单用户锁（`_user_guard`），同群内不同群友可真正并发同时生图（由 `max_concurrent_media_jobs` 全局控制），单人依然保持同时最多 1 个任务防刷屏，彻底解决“同群多人生图报错”的历史瓶颈。
- **改图与视频严格原文直传**：`/g2改图` 与 `/g2视频` 彻底移除提示词处理与视觉资料检索调用，提示词与编辑要求始终原文直传至上游；若检测到 `-off`、`-ex`、`-st`、`-eh`、`-ys`、`-s`、`--search` 等生图专用参数，将在请求发送前直接拦截并返回 `prompt_options_unsupported` 错误。
- **候选模型重试策略可配置**：轮询重试在候选模型间来回切换，依次重试则在当前模型耗尽额外次数后再降级到下一候选；两种策略均遵循不可重试错误立即终止和总任务超时边界。
- **全局统一来源控制与排版**：`show_search_sources`、`max_search_sources` 与 `max_search_output_chars` 统一控制手动指令与会话级 Tool 的来源显示与截断行为。
- **提示词改写英文规范优化**：提示词 `standard`、`enhance` 与预设模式输出更适配 Grok/Flux/SD 底模的地道英文 Prompt，同时强制保留用户指定的引号印刷文字字面量（如 `'不要忘记我'`）与所有否定排除词；`extract_provider_id` 供 `extract` 使用，改写模式共用 `enhance_provider_id`。
- **阶段超时优化与直观配置**：提示词处理默认超时提升至 60 秒（上限 300 秒），视觉资料检索默认超时提升至 120 秒（上限 600 秒），普通搜索提升至 300 秒（上限 900 秒）；任务总超时作为核心配置直观呈现，阶段超时折叠收敛。
- **全链路结构化可读任务日志**：插件加载、面板定时任务调度、生图/改图/视频/搜索与资料检索全面升级为格式化任务块日志，生图明确展示配置默认模式、请求覆盖模式、最终有效模式、提示词状态（已增强/原文直传/回退原文），改图与视频明确展示原文直传说明。

### Fixed

- **非空搜索响应直接接受与误判重试修复**：修复上游深度推理模型（如 `Build/grok-4.5`、`Build/grok-4.6`）在内部思考阶段完成检索并返回数千字有效解答时因未显式包含 `web_search_call` 事件块而被插件误判为未执行搜索并触发重试轮询的问题；现只要返回非空正文即判定成功并立即向用户直出，避免长达数分钟的无效等待与额外费用。
- **细粒度搜索模型尝试与切换日志**：补充搜索候选模型单次尝试开始、实际等待耗时、多模型切换目标与错误原因的 DEBUG 结构化日志。

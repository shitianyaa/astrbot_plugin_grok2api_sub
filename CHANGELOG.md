...
- **任务级搜索请求预算控制 (`max_search_requests_per_task`)**：新增单次任务实际上游搜索请求次数限制（默认 3 次），按 `/v1/responses` 统一扣减，覆盖 `/g2搜索`、LLM 会话 Tool（`grok2api_web_search`）与生图视觉资料搜索，支持 Agent 会话多轮工具调用继承已用预算，防止模型切换与重试过度消耗配额。
...
- **全链路结构化可读任务日志**：插件加载、面板定时任务调度、生图/改图/视频/搜索与资料检索全面升级为格式化任务块日志，生图明确展示配置默认模式、请求覆盖模式、最终有效模式、提示词状态（已增强/原文直传/回退原文）与搜索预算消耗，改图与视频明确展示原文直传说明。

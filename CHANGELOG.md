...
- **文生图五档提示词模式与请求级控制**：`/g2生图` 新增支持 5 档提示词处理模式：`off`（原文直传）、`extract`（仅提取比例/分辨率）、`standard`（精准整理）、`enhance`（受控增强）、`enhance_pro`（深度增强），并支持在命令中以短参数 `-off`、`-ex`、`-st`、`-en`、`-enp`（及长参数 `--off` 等）覆盖单次生图请求模式。
- **生图专用通用视觉事实检索与显式搜索开关 (`-s`/`--search`)**：新增 `character_research_mode`（`off`/`auto`/`always`）与 `character_research_timeout_seconds`。在 `standard`、`enhance`、`enhance_pro` 模式下，`/g2生图` 可自动或通过 `-s`/`--search` 显式检索具名角色、IP、现实物品、建筑、载具等实体的视觉外观事实，清洗后注入改写模型。提示词处理与视觉事实检索**严格仅服务于 `/g2生图`**。
- **显式控制硬失败保障与配置回退自愈**：当使用显式模式参数（如 `-en`）或显式搜索标记（`-s`/`--search`）时，若改写模型异常、显式搜索失败/超时/无资料，任务将立即中止并报告错误，绝不静默回退原文；仅在使用 WebUI 默认配置模式且未显式指定时，才按 `fallback_to_original_on_error` 降级为原文直传。
...
- **配置面板 8 大职责分组与 v3 自动迁移**：将 AstrBot WebUI 插件配置按职责拆分为连接设置、图片与视频、提示词处理、联网搜索、访问控制、性能与可靠性、文件与缓存、管理面板 8 个分组；内置 `config_layout_version` 升级至 3，自动将旧版配置中严格无损行为的 `enhance` 迁移为 `standard`。

### Changed

- **改图与视频严格原文直传**：`/g2改图` 与 `/g2视频` 彻底移除提示词处理与视觉资料检索调用，提示词与编辑要求始终原文直传至上游；若检测到 `-off`、`-ex`、`-st`、`-en`、`-enp`、`-s`、`--search` 等生图专用参数，将在请求发送前直接拦截并返回 `prompt_options_unsupported` 错误。
- **提示词改写英文规范优化**：提示词 `standard`、`enhance`、`enhance_pro` 模式输出更适配 Grok/Flux/SD 底模的地道英文 Prompt，同时强制保留用户指定的引号印刷文字字面量（如 `'不要忘记我'`）与所有否定排除词；`extract_provider_id` 供 `extract` 使用，三种改写模式共用 `enhance_provider_id`。

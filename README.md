...
| 文生图 | 单/多张文生图；支持原文直传、参数提取与 3 档大模型地道英文 Prompt 改写（`standard`/`enhance`/`enhance_pro`），支持联网视觉事实检索与请求级参数控制（`-off`/`-ex`/`-st`/`-en`/`-enp`/`-s`） |
...
| `/g2生图 [模式] [-s] <提示词>` | `/grok2生图` | 访问规则 | 文本生成图片；支持请求级提示词模式（`-off`/`-ex`/`-st`/`-en`/`-enp`）和显式资料搜索（`-s`） |
...
- **`mode`**：`off`（原文直传）、`extract`（仅提取比例/分辨率）、`standard`（精准整理）、`enhance`（受控增强）、`enhance_pro`（深度增强）。仅 `/g2生图` 使用。
- **命令覆盖**：`/g2生图` 可用 `-off`、`-ex`、`-st`、`-en`、`-enp` 覆盖单次请求；参数顺序不敏感，出现多个模式时直接拒绝。
- **`character_research_mode`**：`off`（关闭）、`auto`（识别具名实体后搜索）、`always`（每次生图尝试搜索）；`-s` / `--search` 可在 `standard`、`enhance`、`enhance_pro` 中显式触发。
- **`extract_provider_id` / `enhance_provider_id`**：参数提取使用前者，三种改写模式共用后者。

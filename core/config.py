...
class PluginConfig:
    ...
    def from_astrbot(cls, cmapping: Mapping[str, object]) -> PluginConfig:
        ...
        api_base = _normalize_url("api_base_url", str(g("api_base_url", "")), allow_userinfo=False)
        if not api_base:
            _fail("api_base_url", "必须填写基础地址")
        proxy = _normalize_url(
            "client_proxy_url",
            str(g("client_proxy_url", "http://127.0.0.1:3067")),
            allow_userinfo=True,
        )

        client_key = str(g("client_api_key", "")).strip()

        cfg = cls(
            enabled=_bool_flag("enabled", g("enabled"), True),
            api_base_url=api_base,
            client_api_key=client_key,
            verify_tls=_bool_flag("verify_tls", g("verify_tls"), True),
            client_proxy_url=proxy,
            search_model=str(g("search_model", "")).strip(),
            image_model=str(g("image_model", "")).strip(),
            image_edit_model=str(g("image_edit_model", "")).strip(),
            video_model=str(g("video_model", "")).strip(),
            enable_llm_search_tool=_bool_flag(
                "enable_llm_search_tool", g("enable_llm_search_tool"), True
            ),
            show_search_sources=_bool_flag("show_search_sources", g("show_search_sources"), True),
            max_search_sources=_to_int("max_search_sources", g("max_search_sources", 5), 0, 10),
            max_search_output_chars=_to_int(
                "max_search_output_chars", g("max_search_output_chars", 6000), 500, 20000
            ),
            connect_timeout_seconds=_to_int(
                "connect_timeout_seconds", g("connect_timeout_seconds", 10), 1, 60
            ),
            search_timeout_seconds=_to_int(
                "search_timeout_seconds", g("search_timeout_seconds", 180), 10, 600
            ),
            image_timeout_seconds=_to_int(
                "image_timeout_seconds", g("image_timeout_seconds", 300), 30, 900
            ),
            video_create_timeout_seconds=_to_int(
                "video_create_timeout_seconds", g("video_create_timeout_seconds", 120), 10, 600
            ),
            video_poll_interval_seconds=_to_int(
                "video_poll_interval_seconds", g("video_poll_interval_seconds", 3), 1, 30
            ),
            video_max_wait_seconds=_to_int(
                "video_max_wait_seconds", g("video_max_wait_seconds", 1800), 30, 7200
            ),
            download_timeout_seconds=_to_int(
                "download_timeout_seconds", g("download_timeout_seconds", 300), 30, 1800
            ),
            max_input_image_mb=_to_int("max_input_image_mb", g("max_input_image_mb", 12), 1, 24),
            max_image_download_mb=_to_int(
                "max_image_download_mb", g("max_image_download_mb", 25), 1, 100
            ),
            max_video_download_mb=_to_int(
                "max_video_download_mb", g("max_video_download_mb", 190), 1, 200
            ),
            max_images_per_request=_to_int(
                "max_images_per_request", g("max_images_per_request", 4), 1, 10
            ),
            max_concurrent_searches=_to_int(
                "max_concurrent_searches", g("max_concurrent_searches", 4), 1, 16
            ),
            max_concurrent_media_jobs=_to_int(
                "max_concurrent_media_jobs", g("max_concurrent_media_jobs", 2), 1, 8
            ),
            video_resolution=_to_choice(
                "video_resolution", g("video_resolution", ""), _VIDEO_RESOLUTIONS
            ),
            image_response_format=_to_choice(
                "image_response_format", g("image_response_format", "b64_json"), _IMAGE_FORMATS
            ),
            get_retry_attempts=_to_int("get_retry_attempts", g("get_retry_attempts", 3), 1, 5),
            retry_base_delay_seconds=_to_float(
                "retry_base_delay_seconds", g("retry_base_delay_seconds", 0.5), 0.1, 5.0
            ),
            save_media=_bool_flag("save_media", g("save_media"), False),
            temp_retention_hours=_to_int(
                "temp_retention_hours", g("temp_retention_hours", 24), 1, 168
            ),
            send_video_progress=_bool_flag("send_video_progress", g("send_video_progress"), True),
            user_whitelist=_dedupe(list(g("user_whitelist", []) or []), "user_whitelist"),
            user_blacklist=_dedupe(list(g("user_blacklist", []) or []), "user_blacklist"),
            group_whitelist=_dedupe(list(g("group_whitelist", []) or []), "group_whitelist"),
            group_blacklist=_dedupe(list(g("group_blacklist", []) or []), "group_blacklist"),
            debug_mode=_bool_flag("debug_mode", g("debug_mode"), False),
        )
        return cfg

...
class MediaWorkspace:
    def __init__(self, root: Path, *, max_pixels: int = MAX_PIXELS) -> None:
    ...
    async def image_component_to_data_url(self, component: object) -> str:
        ...
        raw: str = await asyncio.to_thread(convert)
    ...
    # -- cleanup -----------------------------------------------------------
    async def finalize_delivery(self, paths: Sequence[Path], success: bool) -> None:
        """Delete temp files after sending; keep non-empty delivered files."""
        root = self.workspace
        for p in paths:
            try:

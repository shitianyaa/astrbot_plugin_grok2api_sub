...
Separate from the API Key transport on purpose: `HTTPTransport` only permits
``/v1/...`` API key paths, while the management surface lives at
...
from ..common.errors import ConfigurationError, PluginError
...
def _parse_same_origin(base_url: str) -> str:
    """Return ``scheme://authority`` only, ignoring any API Key ``/v1`` suffix.
...
def _validate_read_path(path: str) -> None:
    ...
    Absolute URLs, protocol-relative paths, the API Key ``/v1`` prefix, and any

...
| `presets` | 对象字典 | 自定义风格预设字典，默认提供 `二次元` 与 `电影质感`，可通过 `-ys<名称>` 调用，也可在 WebUI Pages 中视效化管理 |
...
- `/g2生图` 支持在命令中附加短标记（长标记亦支持）覆盖当前请求模式：
  ...
  - `-ys<名称>`：自定义风格预设；直接调用在配置或 WebUI Pages（`pages/presets/`）中定义的专属 System Prompt 指令，如 `-ys二次元`、`-ys电影质感`。
...
| `model_retry_count` | `2` | 搜索、生图、改图候选模型多轮轮询（来回重试）轮数，以及模型目录和图片下载的额外重试次数 |
| `video_retry_count` | `2` | 视频创建候选模型多轮轮询（来回重试）轮数，以及视频状态轮询和视频下载的额外重试次数 |
| `retry_base_delay_seconds` | `0.5` | 指数退避的基础等待，范围 0.1--5.0 |
| `model_switch_errors` | `401,403,404,429,...` | 命中后跳过当前模型剩余重试，切换下一个候选 |

模型重试按轮次（来回）计算：单次尝试失败立即切换至下一个候选模型，遍历完所有候选后进入下一轮，总轮次数为
`1 + retry_count`；生成 POST 也遵循该契约，因此网络错误可能造成重复生成或重复扣费。

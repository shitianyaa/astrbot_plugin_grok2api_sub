...
| 配置键 | 默认值 | 说明 |
|---|---:|---|
| `mode` | `off` | 默认生图模式：`off` 原文直传、`extract` 仅提取参数、`standard` 精准整理、`enhance` 受控增强、`enhance_pro` 深度增强 |
| `extract_provider_id` | `""` | 参数提取模型（`extract` 模式专用），使用 AstrBot 原生供应商选择器 |
| `enhance_provider_id` | `""` | 提示词改写模型（`standard`、`enhance`、`enhance_pro` 模式共享），使用 AstrBot 原生供应商选择器 |
...
- `/g2生图` 支持在命令中附加短标记（长标记亦支持）覆盖当前请求模式：
  ...
  - `-st`（`--standard`）：精准整理；准确翻译并规范整理为英文 Prompt，融合可靠搜索事实，禁止补充未指定的镜头、光影、背景、表情、材质、对象或风格。
  - `-en`（`--enhance`）：受控增强；保留全部显式要求，可补充相容构图、光影、景深、材质和轻微表情，不得新增主体、道具、动作、服装部件、场景或剧情。
  - `-enp`（`--enhance-pro`）：深度增强；在不改变任何显式约束与搜索事实的前提下，可设计镜头视角、构图张力、光影层次、色彩关系、前后景与简洁相容背景。
- 控制标记顺序无关（如 `-s -en` 与 `-en -s` 等价）；若单次请求包含多个模式标记或重复标记，将在请求发送前直接拦截并提示冲突。
- 视觉事实资料检索在 `standard`、`enhance`、`enhance_pro` 模式下支持通过 `-s` 或 `--search` 显式触发；`off` 或 `extract` 下传入 `-s` 会被拒绝；WebUI 中 `character_research_mode` 设为 `off` 时显式 `-s` 也会直接报错。
- 显式指定的控制标记（如 `-en` 或 `-s`）若发生模型异常、搜索失败/超时/无资料，任务将**直接报错中止，绝不静默回退原文**；只有使用 WebUI 默认配置模式且未显式指定标记时，才遵循 `fallback_to_original_on_error` 降级为原文直传。

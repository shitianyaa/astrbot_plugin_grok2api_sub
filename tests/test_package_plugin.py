...
def test_build_package_uses_allowlist(tmp_path: Path, monkeypatch) -> None:
    ...
    result = package_plugin.build_package(root, "v0.1.4", output)
    ...
    assert result["archive"]["sha256"] == package_plugin._sha256_file(archive_path)
...
def test_build_package_is_reproducible_with_source_date_epoch(tmp_path: Path, monkeypatch) -> None:
    ...
    first_result = package_plugin.build_package(root, "v0.1.4", first)
    second_result = package_plugin.build_package(root, "v0.1.4", second)

    assert first_result["archive"]["sha256"] == second_result["archive"]["sha256"]
    assert (first / first_result["archive"]["name"]).read_bytes() == (
        second / second_result["archive"]["name"]
    ).read_bytes()
...
def test_build_package_rejects_version_mismatch_and_existing_output(tmp_path: Path) -> None:
    ...
    with pytest.raises(package_plugin.PackageError, match="metadata.yaml version"):
        package_plugin.build_package(root, "v0.1.5", output)

    package_plugin.build_package(root, "v0.1.4", output)
